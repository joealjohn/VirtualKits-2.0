...
public class KitManager {
    ...
    /**
     * Silent save for enderchest - no messages, used by view GUI
     */
    public void saveECSilent(UUID uuid, int slot, ItemStack[] kit) {
        kitByKitIDMap.put(IDUtil.getECId(uuid, slot), kit);
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> saveEnderchestToDB(uuid, slot));
    }
    ...
    public boolean loadKit(Player player, int slot) {
        ...
        return loadKitInternal(player, IDUtil.getPlayerKitId(player.getUniqueId(), slot),
                "Kit " + slot + " does not exist!", false, () -> {
                    BroadcastManager.get().broadcastPlayerLoadedPrivateKit(player, finalSlot);
                    MessageManager.get().sendMessage(player, MessageManager.get().getKitLoaded(finalSlot));
                    lastKitUsedByPlayer.put(player.getUniqueId(), finalSlot);
                });
    }
    ...
    public boolean loadPublicKit(Player player, String id) {
        return loadKitInternal(player, IDUtil.getPublicKitId(id), "Kit does not exist!", false, () -> {
            ...
            MessageManager.get().sendMessage(player, MessageManager.get().getPublicKitLoaded());
            MessageManager.get().sendRawMessage(player,
                    "<gray>ʏᴏᴜ ᴄᴀɴ ꜱᴀᴠᴇ ᴀ ᴄᴜꜱᴛᴏᴍ ᴠᴇʀꜱɪᴏɴ ᴛʜɪꜱ ᴋɪᴛ ʙʏ ɪᴍᴘᴏʀᴛɪɴɢ ɪɴᴛᴏ ᴛʜᴇ ᴋɪᴛ ᴇᴅɪᴛᴏʀ");
        });
    }
    ...
    public boolean loadEnderchest(Player player, int slot) {
        ...
        return loadKitInternal(player, IDUtil.getECId(player.getUniqueId(), slot),
                "Enderchest " + slot + " does not exist!", true, () -> {
                    BroadcastManager.get().broadcastPlayerLoadedEnderChest(player, finalSlot);
                    MessageManager.get().sendMessage(player, MessageManager.get().getEcLoaded(finalSlot));
                });
    }
    ...
    public void loadPlayerDataFromDB(UUID uuid) {
        for (int slot = 1; slot < 10; slot++) {
            ...
            if (data != null && !data.isEmpty() && !data.equalsIgnoreCase("error")) {
                try {
        ...
        for (int slot = 1; slot < 10; slot++) {
            String data = VirtualKits.storageManager.getKitDataByID(IDUtil.getECId(uuid, slot));
            if (data != null && !data.isEmpty() && !data.equalsIgnoreCase("error")) {
                try {
                    ItemStack[] kit = Serializer.itemStackArrayFromBase64(data);
    ...
    private void saveKitToDB(String key, boolean removeAfterSave) {
        if (kitByKitIDMap.get(key) != null) {
            VirtualKits.storageManager.saveKitDataByID(key,
    ...
    public void loadPublicKitFromDB(String id) {
        String data = VirtualKits.storageManager.getKitDataByID(IDUtil.getPublicKitId(id));
        if (data != null && !data.isEmpty() && !data.equalsIgnoreCase("error")) {
            try {
                ItemStack[] kit = Serializer.itemStackArrayFromBase64(data);
    ...
    public boolean deleteKit(UUID uuid, int slot) {
        if (hasKit(uuid, slot)) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(plugin,
                    () -> VirtualKits.storageManager.deleteKitByID(IDUtil.getPlayerKitId(uuid, slot)));
            return true;
        }
        return false;
    }
    ...
    public boolean deleteEnderchest(UUID uuid, int slot) {
        if (hasEC(uuid, slot)) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(plugin,
                    () -> VirtualKits.storageManager.deleteKitByID(IDUtil.getECId(uuid, slot)));
            return true;
        }
        return false;
    }
