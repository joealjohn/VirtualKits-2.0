...
public class SoundManager {
    ...
    private static Sound getSound(String key, String defaultName) {
        ...
        String soundName = VirtualKits.getPlugin().getConfig().getString(path, defaultName);
        try {
            return Sound.valueOf(soundName);
        } catch (IllegalArgumentException ex) {
            VirtualKits.getPlugin().getLogger()
    ...
    private static void play(Player player, Sound sound) {
        ...
        if (!VirtualKits.getPlugin().getConfig().getBoolean("sounds.enabled", true)) {
            return;
        }
